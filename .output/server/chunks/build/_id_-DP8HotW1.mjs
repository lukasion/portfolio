import { _ as _sfc_main$1, a as _sfc_main$2 } from './Footer-9Auhc8Y7.mjs';
import { _ as __nuxt_component_1 } from './Short-CoJfDXx5.mjs';
import { d as defineStore, a as useI18n, b as useRoute } from './server.mjs';
import { defineComponent, unref, useSSRContext, ref } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrInterpolate, ssrRenderStyle, ssrRenderList } from 'vue/server-renderer';
import { u as useArticlesStore } from './articles-D5VY78IW.mjs';
import { u as useFetch } from './fetch-CCjfMeZS.mjs';
import './nuxt-link-DKVwpM5a.mjs';
import './Icon-kqhLPqFV.mjs';
import '@iconify/vue/dist/offline';
import '@iconify/vue';
import './index-yzm_4dp1.mjs';
import './_plugin-vue_export-helper-1tPrXgE0.mjs';
import 'vue-router';
import '../runtime.mjs';
import 'node:http';
import 'node:https';
import 'fs';
import 'path';
import 'requrl';
import 'node:fs';
import 'node:url';
import 'redirect-ssl';
import 'nodemailer';
import '@dword-design/functions/dist/find-index.js';
import '@dword-design/functions/dist/omit.js';
import 'unhead';
import '@unhead/shared';
import 'dayjs';
import 'dayjs/plugin/updateLocale.js';
import 'dayjs/plugin/timezone.js';
import 'dayjs/plugin/relativeTime.js';
import 'dayjs/plugin/utc.js';
import 'tailwind-merge';

const useCategoriesStore = defineStore("categories", () => {
  const category = ref(null);
  async function fetchCategory(id) {
    const { data } = await useFetch(`/api/categories/${id}`, "$V7dYtnPUl4");
    if (data.value) {
      category.value = data.value;
    }
  }
  return {
    category,
    fetchCategory
  };
});
const _sfc_main = /* @__PURE__ */ defineComponent({
  __name: "[id]",
  __ssrInlineRender: true,
  setup(__props) {
    const articlesStore = useArticlesStore();
    const categoriesStore = useCategoriesStore();
    const i18n = useI18n();
    const route = useRoute();
    articlesStore.fetchData({
      limit: 4,
      lang: i18n.locale.value,
      categoryId: route.params.id
    });
    categoriesStore.fetchCategory(route.params.id);
    return (_ctx, _push, _parent, _attrs) => {
      var _a;
      const _component_Header = _sfc_main$1;
      const _component_article_short = __nuxt_component_1;
      const _component_Footer = _sfc_main$2;
      _push(`<div${ssrRenderAttrs(_attrs)}>`);
      _push(ssrRenderComponent(_component_Header, { white: "" }, null, _parent));
      _push(`<div class="container mx-auto pt-24 md:pt-32 px-4"><h1 class="text-2xl font-bold">${ssrInterpolate(unref(i18n).t("blog"))}</h1><p class="text-md md:text-xl mt-4 mb-4">${ssrInterpolate(unref(i18n).t("blog_subtitle"))}</p><div class="blog__container"><div class="blog__articles"><h3 class="text-md md:text-2xl mt-8 mb-4">${ssrInterpolate(unref(i18n).t("articles_in_category"))}: <span style="${ssrRenderStyle({ "font-weight": "500" })}">${ssrInterpolate((_a = unref(categoriesStore).category) == null ? void 0 : _a.name)}</span></h3><div class="flex flex-col gap-12">`);
      if (unref(articlesStore).articles.length > 0) {
        _push(`<!--[-->`);
        ssrRenderList(unref(articlesStore).articles, (article) => {
          _push(ssrRenderComponent(_component_article_short, {
            article,
            column: ""
          }, null, _parent));
        });
        _push(`<!--]-->`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div></div></div>`);
      _push(ssrRenderComponent(_component_Footer, { class: "mt-24" }, null, _parent));
      _push(`</div>`);
    };
  }
});
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/category/[id].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_id_-DP8HotW1.mjs.map
