  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(articlesStore).articles.length > 0) {
        _push(`<table class="table table-zebra border border-gray-200 rounded-md shadow mt-4"><thead><tr><th>ID</th><th>Title</th><th>Content</th><th>Actions</th></tr></thead><tbody><!--[-->`);
        ssrRenderList(unref(articlesStore).articles, (article) => {
          _push(`<tr><td>${ssrInterpolate(article.id)}</td><td>${ssrInterpolate(article.title)}</td><td>${ssrInterpolate(("shorten" in _ctx ? _ctx.shorten : unref(shorten))(article.content, 150))}</td><td><a class="btn btn-sm btn-neutral"${ssrRenderAttr("href", `/blog/${article.friendly_url}`)} target="_blank">Preview</a><a class="btn btn-sm btn-neutral"${ssrRenderAttr("href", `/user/posts/form/${article.id}`)}>Edit</a></td></tr>`);
        });
        _push(`<!--]--></tbody></table>`);
      } else {
        _push(`<!---->`);
...
//# sourceMappingURL=_slug_-EHtIwjyx.mjs.map
